 ...
 * менша ніж значення параметра threshold.
 */
...
const sortByDescendingPrice = cars => [...cars].sort((prev, next) => next.price - prev.price);
...
const sortByModel = (cars, order) =>
  [...cars].sort((prev, next) =>
    order === 'asc' ? prev.model.localeCompare(next.model) : next.model.localeCompare(prev.model)
  );
...
const getModelsOnSale = cars =>
  cars.reduce((acc, car) => (car.onSale ? [...acc, car.model] : acc), []);
...
/*
 * Нехай функція getSortedCarsOnSale повертає масив автомобілів на
