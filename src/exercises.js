...
function logItems(items) {
  ...
  // 2.
  items.forEach((value, idx, arr) => {
    console.log(`${idx + 1} - ${value}`);
  });
}

// 3.
//   items.forEach((value, idx, arr) => console.log(`${idx + 1} - ${value}`));
// }
...
function printContactsInfo(names, phones) {
  ...
  for (let i = 0; i < nameArr.length; i += 1) {
    ...
    console.log(`${name} - ${phoneArr[i]}`);
  }

  // 2.
  // nameArr.forEach((name, idx) => {
  //   console.log(`${name} - ${phoneArr[idx]}`);
  // });
}
...
updateCourse('Express', 'NestJS'); // 'Курс із таким ім'ям не знайдено'
